...
int CXHeaderCtrl::DrawImage(CDC* pDC, CRect rect, LPHDITEM lphdi, BOOL bRight)
{
	...
	if (lphdi->iImage != XHEADERCTRL_NO_IMAGE)
	{
		...
		else if (rect.Width() > 0)
		{
			...
			// fill rect inside checkbox with white or gray
			COLORREF rgbBackground;
			if (lphdi->iImage == XHEADERCTRL_SOME_CHECKED_IMAGE)		// some items checked. BFish added 2010.10
			{
				rgbBackground = pDC->SetBkColor(RGB(204,204,204));
				pDC->FillSolidRect(&chkboxrect, RGB(204,204,204));
			}
			else
			{
				rgbBackground = pDC->SetBkColor(::GetSysColor(COLOR_WINDOW));
			...
			if (lphdi->iImage==XHEADERCTRL_CHECKED_IMAGE || lphdi->iImage==XHEADERCTRL_SOME_CHECKED_IMAGE)
			{
...
{
	TRACE0("in CXHeaderCtrl::OnSysColorChange\n");
...
{
	TRACE0("in CXHeaderCtrl::OnLButtonDblClk\n");
