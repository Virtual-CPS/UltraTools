...
CAdvComboBox::CAdvComboBox( BOOL bInst, CWnd * pParent ) :
	...
	m_pParent(pParent)
{
	TRACE0(_T("in CAdvComboBox::CAdvComboBox\n"));
	RegisterWindowClass();
	m_pEdit = NULL;
	m_zDelta = 0;
	m_nCurSel = -1;
...
{
	TRACE0(_T("in CAdvComboBox::Create\n"));
	m_rcCombo = rect;
	m_bCodeCreate = true;
...
{
	TRACE0(_T("in CAdvComboBox::OnCreate\n"));
...
{
	TRACE0(_T("in CAdvComboBox::PreSubclassWindow\n"));
...
{
	TRACE0(_T("in CAdvComboBox::OnDropdownButton\n"));
	//
	//
	if( !m_bDropListVisible )
	{
...
{
	TRACE0(_T("in CAdvComboBox::OnDestroyDropdownList\n"));
	//
	// 
	if( m_pDropWnd )
	{
...
{
	TRACE0(_T("in CAdvComboBox::CreateDropList\n"));
	CRect rc;
	if( m_pDropWnd )
		ASSERT(0);
...
{
	TRACE0(_T("in CAdvComboBox::OnKillFocus\n"));
...
int CAdvComboBox::FindStringExact(int nIndexStart, LPCTSTR lpszFind)
{
	...
	for( m_iter; m_iter != m_list.end(); ++m_iter )
	{
		...
		{
			TRACE0(_T("CAdvComboBox::FindStringExact returning %d\n"), nPos);
			return nPos;
		}
		nPos++;
	}
...
BOOL CAdvComboBox::PreTranslateMessage(MSG* pMsg) 
{
	if( pMsg->message == WM_KEYDOWN )
	{
		...
		TRACE0(_T("CAdvComboBox: Key was pressed(AdvComboBox)\n"));

		if( pMsg->wParam == VK_RETURN )
		{
			TRACE0(_T("CAdvComboBox: VK_RETURN\n"));
			if( m_pDropWnd )
			{
		...
		{
			TRACE0(_T("CAdvComboBox: VK_ESCAPE\n"));
			if( m_pDropWnd )
			{
				OnDestroyDropdownList(0,0);
				Invalidate();
...
{
	TRACE0(_T("in CAdvComboBox::SelNextPage\n"));
	if (m_pDropWnd)
	{
...
{
	TRACE0(_T("in CAdvComboBox::OnKeyDown\n"));
...
{
	TRACE0(_T("in CAdvComboBox::OnActivateApp\n"));
