...
// add by BFish 2010.10
void CloneXListCtrlData(XLISTCTRLDATA& dstData, const XLISTCTRLDATA& srcData)
{
	dstData.bEnabled             = srcData.bEnabled;
	dstData.bBold                = srcData.bBold;
	dstData.bModified            = srcData.bModified;
	dstData.nImage               = srcData.nImage;
#ifndef NO_XLISTCTRL_TOOL_TIPS
	dstData.strToolTip           = srcData.strToolTip;
#endif
	dstData.bCombo               = srcData.bCombo;
	dstData.bSort                = srcData.bSort;
	dstData.psa                  = srcData.psa;
	dstData.nComboListHeight     = srcData.nComboListHeight;
	dstData.nInitialComboSel     = srcData.nInitialComboSel;
	dstData.bEdit                = srcData.bEdit;
	dstData.crText               = srcData.crText;
	dstData.crBackground         = srcData.crBackground;

	dstData.bShowProgress        = srcData.bShowProgress;
	dstData.nProgressPercent     = srcData.nProgressPercent;
	dstData.strProgressMessage   = srcData.strProgressMessage;
	dstData.bShowProgressMessage = srcData.bShowProgressMessage;
	dstData.nCheckedState        = srcData.nCheckedState;
	dstData.dwItemData           = srcData.dwItemData;
}
...
BEGIN_MESSAGE_MAP(CXListCtrl, CListCtrl)
	...
	ON_WM_RBUTTONDOWN()
	ON_WM_NCLBUTTONDOWN()
	ON_WM_MEASUREITEM_REFLECT()		// add by BFish 2010.10
	//}}AFX_MSG_MAP
...
{
	m_nItemHeight = 10;			// add by BFish 2010.10
							  ...
							  XLISTCTRLDATA *pXLCD)
{
	...
	CRect rcCheckBox;
	rcCheckBox = rect;
	int nHeiht = rect.Height();
	if (rect.Width() <= 13)
	{
		rcCheckBox.bottom -= 1;
		rcCheckBox.left += 9;			// line up checkbox with header checkbox
		nHeiht = rcCheckBox.Height();
	}
	else
	{
		rcCheckBox.bottom = rcCheckBox.top + 13;
		nHeiht = rcCheckBox.Height();				// ==13
	}
	rcCheckBox.right = rcCheckBox.left + nHeiht;	// width = height
	...
	{
		// center the checkbox
		rcCheckBox.left = rect.left + rect.Width()/2 - nHeiht/2 - 1;
		rcCheckBox.right = rcCheckBox.left + nHeiht;

		rcCheckBox.top = rect.top + rect.Height()/2 - nHeiht/2 -1;
		rcCheckBox.bottom = rcCheckBox.top + nHeiht;
	}
	...
	pDC->FrameRect(&rcCheckBox, &brush);
	...
	if (pXLCD[nSubItem].nCheckedState == 1)
	{
		...
		// draw the checkmark
		int x = rcCheckBox.left + 9;
		ASSERT(x < rcCheckBox.right);
		int y = rcCheckBox.top + 3;
		int i;
		for (i = 0; i < 4; i++)
		{
			pDC->MoveTo(x, y);
	...
	if (!str.IsEmpty())
	{
		...
		CRect textrect;
		textrect = rect;
		textrect.left = rcCheckBox.right + 4;
...
{
	TRACE0("in CXListCtrl::OnLButtonDown\n");
	...
	{
		TRACE2("lvhit.iItem=%d  lvhit.iSubItem=%d  ~~~~~\n", lvhit.iItem, lvhit.iSubItem);
...
{
	TRACE0("in CXListCtrl::OnRButtonDown\n");
...
{
	TRACE("in CXListCtrl::OnNcLButtonDown\n");
...
int CXListCtrl::InsertItem(const LVITEM* pItem)
{
	...
	pXLCD[0].dwItemData   = (DWORD)(pItem->lParam);	//+++
...
// add by BFish
XLISTCTRLDATA* CXListCtrl::GetItemData(int nItem, DWORD& dwItemData) const
{
	ASSERT(nItem >= 0);
	ASSERT(nItem < GetItemCount());
	if ((nItem < 0) || nItem >= GetItemCount())
...
BOOL CXListCtrl::SetHeaderCheckedState(int nSubItem, int nCheckedState)
{
	...
	if ((nSubItem < 0) || nSubItem >= GetColumns())
		return FALSE;
	ASSERT(nCheckedState==XHEADERCTRL_NO_IMAGE ||				// BFish changed the const to macro
			nCheckedState==XHEADERCTRL_UNCHECKED_IMAGE ||
			nCheckedState==XHEADERCTRL_CHECKED_IMAGE ||
			nCheckedState==XHEADERCTRL_SOME_CHECKED_IMAGE);
...
{
	TRACE0("in CXListCtrl::OnColumnClick\n");
...
// 		m_HeaderCtrl.UpdateWindow();
		...
		int nItem, nCnt;
		nCnt = GetItemCount();
		BOOL bSomeNotChecked = FALSE;
		for (nItem = 0; nItem < nCnt; nItem++)
		{
			XLISTCTRLDATA *pXLCD = (XLISTCTRLDATA *) CListCtrl::GetItemData(nItem);
			if (!pXLCD)
			{
			...
			//if (pXLCD[nSubItem].nCheckedState != -1)
			if (pXLCD[nSubItem].nCheckedState!=-1 && GetEnabled(nItem))	// BFish modified 2011.07.10
			{
				...
				bSomeNotChecked = TRUE;
		}

		if(bSomeNotChecked && nCheckedState==XHEADERCTRL_CHECKED_IMAGE)
			SetHeaderCheckedState(nSubItem, XHEADERCTRL_SOME_CHECKED_IMAGE);
		else
			SetHeaderCheckedState(nSubItem, nCheckedState);
		m_HeaderCtrl.UpdateWindow();
	}
...
// Abstracted from SetCheckbox() and OnLButtonDown(). BFish 2010.10
void CXListCtrl::UpdateHeaderCheckbox(int nSubItem)
{
	ASSERT(nSubItem >= 0);
	ASSERT(nSubItem < GetColumns());
	if ((nSubItem < 0) || nSubItem >= GetColumns())
	...
	// BFish change the checkbox state from 2 styles to 3 styles 2010.10
...
{
	TRACE0("in CXListCtrl::OnSysColorChange\n");
...
{
	TRACE0("in CXListCtrl::OnComboEscape\n");
	SetTimer(2, 50, NULL);
	//UpdateSubItem(m_nComboItem, m_nComboSubItem);
	return 0;
}
...
{
	TRACE0("in CXListCtrl::OnComboComplete\n");
	SetTimer(3, 50, NULL);
	return 0;
}
...
int CXListCtrl::OnToolHitTest(CPoint point, TOOLINFO * pTI) const
{
	...
	//TRACE0(_T("in CToolTipListCtrl::OnToolHitTest: %d,%d\n"), nItem, nSubItem);
...
BOOL CXListCtrl::OnToolTipText(UINT /*id*/, NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	//TRACE0(_T("in CXListCtrl::OnToolTipText: id=%d\n"), nID);
...
void CXListCtrl::DrawComboBox(int nItem, int nSubItem)
{
	...
	{
		TRACE0("XListCtrl: combo boxes require LVS_EX_FULLROWSELECT style\n");
		ASSERT(FALSE);
	}
#endif
	...
	if (m_pCombo)
	{
		...
		if (bSuccess)
		{
			...
			if (pXLCD[nSubItem].psa)
			{
				...
				{
					TRACE("ERROR - exception in CXListCtrl::DrawComboBox\n");
					TRACE("==>  Attempting to access psa pointer;  string array must be a class\n");
					TRACE("==>  variable, a global variable, or allocated on the heap.\n");
				}
			}
			...
			if (pXLCD[nSubItem].psa)
			{
				try
				{
						...
						(m_pCombo->GetCount() > pXLCD[nSubItem].nInitialComboSel))
					{
						...
						TRACE2("nInitialComboSel=%d  str=<%s>\n", index, str);
						SetItemText(nItem, nSubItem, str);
			...
			else
			{
				...
				TRACE1("FindStringExact returned %d\n", index);
...
void CXListCtrl::DrawEdit(int nItem, int nSubItem)
{
	...
	if ((dwExStyle & LVS_EX_FULLROWSELECT) == 0)
	{
		TRACE0("XListCtrl: edit boxes require LVS_EX_FULLROWSELECT style\n");
		ASSERT(FALSE);
	}
#endif
...
{
	TRACE0("in CXListCtrl::OnXEditEscape\n");
...
{
	TRACE0("in CXListCtrl::OnXEditKillFocus\n");
...
{
	TRACE0("in CXListCtrl::OnKeyDown\n");
