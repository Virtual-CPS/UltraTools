...
CCreditStatic::CCreditStatic()
{
	...
	m_Colors[0] = RGB(255,255,255);		// White
	m_Colors[1] = RGB(0,0,255);			// Blue
	m_Colors[2] = RGB(255,0,0);			// Red
	m_Colors[3] = RGB(0,128,255);		// Turquoise
...
BOOL CCreditStatic::StartScrolling()
{
	...
	TimerOn = (UINT)SetTimer(DISPLAY_TIMER_ID,m_DisplaySpeed[m_CurrentSpeed],NULL);
    ASSERT(TimerOn != 0);
	...
	while(pos != NULL && height <= m_ScrollRect.Height())
	{
		...
		{				
			...
			if(m_szWork.GetLength()-rmcode != 0) 
			{
				...
				size = memDC.GetTextExtent(_T("W"),1);
			height += size.cy;
		}
		else
		{
