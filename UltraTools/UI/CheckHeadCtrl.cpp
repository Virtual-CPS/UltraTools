...
BEGIN_MESSAGE_MAP(CCheckHeadCtrl, CBCGPHeaderCtrl)
...
void CCheckHeadCtrl::OnItemClicked(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	BOOL bCheck = hdItem.iImage == 2 ? TRUE : FALSE;
	CBCGPListCtrl* pListCtrl = (CBCGPListCtrl*)GetParent();
	int nCount = pListCtrl->GetItemCount();	
	for(nItem = 0; nItem < nCount; nItem++)
	{
		ListView_SetCheckState(pListCtrl->GetSafeHwnd(), nItem, bCheck);
	}
}